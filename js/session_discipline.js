 ...
 * Spezialisiertes Modul für den Bestrafungs-Konfigurator im Schlafzimmer-Cockpit.
 * 
 * Features:
...
(function(window) {
  ...
  function getBottomQuestionnaireMatches(infractionCat, severity) {
    ...
    var allAnswers = (typeof window.answers !== 'undefined' && window.answers[subKey]) ? window.answers[subKey] : {};
    ...
    // Nach Bewertung sortieren (Noten 5 und 4 zuerst, gefolgt von 2 als echter Buße)
  ...
  function getStageOptions(stage) {
    ...
    var sev = wizardState.severities[stage];
    ...
    if (stage === 1) {
      ...
      // Echte Bogen-Treffer einbinden
      var realMatches = getBottomQuestionnaireMatches(wizardState.category, sev);
      var cards = [];

      // 1. KI- / Regie-Option basierend auf Freitext
      var reasonText = wizardState.reason.trim();
      ...
      // 2. Bis zu 3 reale Bogen-Optionen des Bottoms ergänzen
      if (realMatches.length > 0) {
        ...
        // Fallback wenn noch kein Bogen ausgefüllt wurde
        cards.push(
  ...
  function validateCurrentPhysicalSetup() {
    ...
    // Zusätzliche Inline-Plausibilitätsprüfung
    var handsBack = setup.bondage && (setup.bondage.id === 'leather_wrist_cuffs' || setup.posture?.id === 'hands_behind_back');
  ...
  function renderStageCards(stage) {
    ...
    container.innerHTML = options.map(function(opt, idx) {
      var isSel = wizardState.selectedChoices[stage]?.id === opt.id || (!wizardState.selectedChoices[stage] && idx === 0);
      if (isSel && !wizardState.selectedChoices[stage]) {
  ...
  function renderDisciplineSummary() {
    ...
    var act = wizardState.selectedChoices[1]?.title || 'Disziplinierung';
    var pos = wizardState.selectedChoices[2]?.title || 'Kniestand';
    var bon = wizardState.selectedChoices[3]?.title || 'Keine Fesselung';
    var sen = wizardState.selectedChoices[4]?.title || 'Keine sensorische Einschränkung';
    var reason = wizardState.reason.trim() || 'Verstoß gegen Vereinbarungen';
  ...
  function openModal() {
    ...
    showStage(1);
    document.getElementById('modal-incident-discipline')?.classList.remove('hidden');
  }

  function closeModal() {
    document.getElementById('modal-incident-discipline')?.classList.add('hidden');
  }
  ...
  window.SessionDiscipline = {
    ...
    handleReasonInput: function(val) {
      ...
      var ratEl = document.getElementById('top-assist-rationale');
      if (ratEl) {
        ratEl.innerText = val.trim().length > 3
          ? "Anlass erfasst: \"" + val.trim() + "\". Kontext fließt in die Haltung & Zucht ein."
          : (INFRACTION_MAP[wizardState.category]?.hint || "Wähle oder beschreibe ein Vergehen.");
      }
      renderStageCards(1);
    },
